...
| Bússola de campo | **Implementado** | Leitura do sensor, azimute verdadeiro e de grade, rumo e distância ao destino com lado a corrigir, conferência pelo Sol, declinação manual e rumo travado (ADR-0040). | A referência do sensor depende do modelo: sem correção medida o app não afirma azimute verdadeiro. Não há modelo magnético embarcado — a declinação é medida pelo Sol ou informada. |
| Escuta de ambiente | **Implementado** | Analisador de frequências pelo microfone: avisa por vibração quando o grave sobe como sobe um veículo se aproximando, ou quando alguém chama em voz alta. Espectro, piso do lugar e inclinação na tela; três réguas de sensibilidade (ADR-0041). | **Só recebe — nunca transmite, grava ou guarda áudio**, e há teste estrutural cobrando isso. Não diz de que lado vem (exigiria dois microfones). Não usa a antena do celular: rádio não é acessível a um aplicativo. Com a tela apagada o sistema pode suspender o áudio. Os limiares são ponto de partida calculado, não medição de estrada. |
