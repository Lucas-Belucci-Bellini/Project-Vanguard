...
| V2-003 | GPS/GNSS | Posição, erro e indisponibilidade | `src/core/localizacao.js` | Testes de normalização/idade; aparelho real pendente | README, checklist mobile | IN_PROGRESS | 130 testes; hardware pendente |
...
| V2-009 | Offline maps | Preparar/consultar/limpar tiles | Cache HTTPS limitado a 256 URLs/preparação; planner e Service Worker removem templates/URLs duplicados e exigem allowlist HTTPS | Cobertura real, quota, resposta dos provedores e modo avião pendentes | README, plano de campo, ADR-0011/0012 | IN_PROGRESS | 133 testes; limite/antimeridiano/allowlist/deduplicação locais; não é cobertura completa |
...
| V2-019 | Diagnostics | Mostrar estado de app, GPS, rede, cache, bateria e lifecycle | `src/core/diagnostico.js`, `src/core/ciclo-vida.js`, `src/pages/diagnostico.js`, rota `#/diagnostico` | Testes de diagnóstico/lifecycle/performance + preview; validação física pendente | `V2_STATUS.md`, ADR-0005/0008, README | IN_PROGRESS | 130 testes totais; browser validado; device pending |
...
| V2-021 | Performance | Medir startup, mapa, memória e bundle | `desempenhoResumo()` no Diagnóstico + `chaveDesenhoGrade()` para deduplicar rótulos repetidos do Mapa | 5 testes puros; preview do Mapa; profiling de FPS/bateria/memória do sistema pendente | ADR-0008/0009, referências e validações de performance/mapa | IN_PROGRESS | 129 testes; render deduplicado; sem profiling físico completo |
