...
| Tests | `npm test` | Aprovado | 176 testes; inclui localização manual, catálogo de rotas e cleanup da centralização | não substitui validação física |
...
| Android debug | `npm run mobile:android:debug` | Aprovado | APK debug de teste regenerado no commit funcional `6d7c7fb`; tamanho/hash devem ser registrados por execução | não é release nem distribuição |
