...
function rotuloDaLinha(valor, passo) { return String(Math.floor((valor % 100000) / passo) % 100).padStart(2, '0'); }
...
export function mapaPage() {
  ...
  alvoMapa.append(rotulos); raiz.append(alvoMapa, hud, painel);

  let mapa = null; let motorMapa = null; let watchId = null; let desmontado = false; let redesenhar = null;
  let peca = estado.get(CHAVES.PECA, null); let alvo = estado.get(CHAVES.ALVO, null); let modoClique = null; let posGps = null;
  ...
  const infoPeca = h('div', { className: 'mapa__info' }, '—'); const infoAlvo = h('div', { className: 'mapa__info' }, '—'); const infoSolucao = h('div', { className: 'mapa__solucao' });
  ...
  function setModo(m) { modoClique = modoClique === m ? null : m; btnPeca.classList.toggle('primario', modoClique === 'peca'); btnAlvo.classList.toggle('primario', modoClique === 'alvo'); if (mapa) mapa.getCanvas().style.cursor = modoClique ? 'crosshair' : ''; }
  function descrever(p) { if (!p) return '—'; return h('div', null, h('div', { className: 'mapa__info-mgrs' }, latLonParaMGRS(p.lat, p.lon, 5, true)), h('div', { className: 'u-mudo' }, `${num(p.lat, 6)}, ${num(p.lon, 6)} · ALT ${p.alt == null ? '—' : `${num(p.alt, 0)} m`}`)); }
  ...
  async function marcar(tipo, lngLat) {
    const p = { lat: lngLat.lat, lon: lngLat.lng, alt: null }; if (tipo === 'peca') peca = p; else alvo = p;
    atualizarMarcadores(); atualizarLeituras(); const alt = await buscarAltitude(p.lat, p.lon); if (desmontado) return; p.alt = alt ?? 0;
    estado.set(tipo === 'peca' ? CHAVES.PECA : CHAVES.ALVO, p); estado.set(CHAVES.QUADRO, 'geo'); atualizarLeituras();
  }
  function alternarGps() {
    ...
    watchId = navigator.geolocation.watchPosition((pos) => {
      posGps = { lat: pos.coords.latitude, lon: pos.coords.longitude, acc: pos.coords.accuracy }; statusGps.textContent = `Fixo · precisão ±${Math.round(posGps.acc)} m`; atualizarMarcadores();
  ...
  (async () => {
    try {
      ...
      if (!desmontado) raiz.append(h('div', { className: 'vg-aviso vg-aviso--perigo mapa__falha' }, `Não foi possível carregar o motor de mapa: ${erro.message}`));
      return;
    }
    ...
    mapa.on('load', () => {
      ...
      const css = getComputedStyle(document.documentElement); const corGrade = css.getPropertyValue('--grid-line').trim() || 'rgba(220,214,192,0.22)'; const corGradeForte = css.getPropertyValue('--grid-line-forte').trim() || 'rgba(220,214,192,0.4)';
    ...
    function desenharRotulos() {
      if (!mapa) return;
      const dpr = window.devicePixelRatio || 1, largura = alvoMapa.clientWidth, altura = alvoMapa.clientHeight; if (!largura || !altura) return;
      ...
      const css = getComputedStyle(document.documentElement); const corGrid = css.getPropertyValue('--grid-label').trim() || '#ffb000'; const corTexto = css.getPropertyValue('--color-text-primary').trim() || '#dcd6c0'; const corFundo = css.getPropertyValue('--color-bg').trim() || '#0c0f0a'; const fonte = css.getPropertyValue('--font-mono').trim() || 'monospace';
      ...
      for (const f of gradeAtual.features) {
        ...
        if (f.properties.eixo === 'E') { const topo = mapa.project(coords[coords.length - 1]), base = mapa.project(coords[0]); if (topo.x > 14 && topo.x < largura - 14) escrever(txt, topo.x, 14, corGrid); if (base.x > 14 && base.x < largura - 14) escrever(txt, base.x, altura - 12, corGrid); }
        else { const esq = mapa.project(coords[0]), dir = mapa.project(coords[coords.length - 1]); if (esq.y > 14 && esq.y < altura - 14) escrever(txt, 16, esq.y, corGrid); if (dir.y > 14 && dir.y < altura - 14) escrever(txt, largura - 16, dir.y, corGrid); }
      }
      for (const m of [peca && { p: peca, t: 'PEÇA' }, alvo && { p: alvo, t: 'ALVO' }, posGps && { p: posGps, t: 'VOCÊ' }].filter(Boolean)) { const pt = mapa.project([m.p.lon, m.p.lat]); if (pt.x < -50 || pt.x > largura + 50 || pt.y < -50 || pt.y > altura + 50) continue; escrever(m.t, pt.x, pt.y + 18, corTexto, 10); }
    }
    mapa.on('moveend', redesenharGrade); mapa.on('zoomend', redesenharGrade); mapa.on('render', desenharRotulos); redesenhar = desenharRotulos;
    mapa.on('mousemove', (e) => { try { hudMgrs.textContent = latLonParaMGRS(e.lngLat.lat, e.lngLat.lng, 5, true); hudLatLon.textContent = `${num(e.lngLat.lat, 6)}, ${num(e.lngLat.lng, 6)}`; const u = latLonParaUTM(e.lngLat.lat, e.lngLat.lng); hudZona.textContent = `FUSO ${u.zona}${u.banda} · E ${Math.round(u.easting)} N ${Math.round(u.northing)}`; } catch { hudMgrs.textContent = 'FORA DA COBERTURA UTM'; hudLatLon.textContent = ''; hudZona.textContent = ''; } });
    mapa.on('click', (e) => { if (!modoClique) return; marcar(modoClique, e.lngLat); setModo(null); });
    selBase.onchange = () => {
      const id = selBase.value;
      for (const base of Object.keys(BASES)) { const layerId = `base-${base}`; if (mapa.getLayer(layerId)) mapa.setLayoutProperty(layerId, 'visibility', base === id ? 'visible' : 'none'); }
    };
  })();

  function desmontar() { desmontado = true; if (watchId != null) { navigator.geolocation.clearWatch(watchId); watchId = null; } if (motorMapa) { try { motorMapa.desmontar(); } catch {} motorMapa = null; mapa = null; } else if (mapa) { try { mapa.remove(); } catch {} mapa = null; } }
