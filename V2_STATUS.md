...
| **Last Completed Task** | Adaptador de leitura de permissão GPS integrado ao Diagnóstico, usando Capacitor nativo ou Permissions API Web sem prompt automático |
...
| **Tests** | `npm test`: 150 testes aprovados; `node --check public/sw.js`, `git diff --check` e auditoria de produção aprovados; permissões, compartilhamento, capacidades e diagnóstico `lat/lon` cobertos |
