...
| **Current Task** | Definir backend durável, atomicidade apropriada e fontes/licenças para o futuro dataset; usar `OFFLINE_DATA_STATUS.md`, `MAP_DATA_STATUS.md` e `SYNC_STATUS.md` sem inventar cobertura mundial offline |
| **Last Completed Task** | Adapter local isolado para manifesto ativo e transação, com envelopes, validação e falhas explícitas; `4aa6556`, CI `33131381528` concluído com sucesso |
...
| **Tests** | `npm test`: 194 testes aprovados; manifesto, transação e storage isolado de dataset, cleanups assíncronos, fixo manual, configuração pública, atualização, camadas, Service Worker, catálogo, registros e diagnóstico cobertos |
